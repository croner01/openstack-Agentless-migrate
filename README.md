...
**「卷拷贝」页签**（只讲 RBD 路径，读 `vm.volumes`）与**「中转机通道」页签**
（读中转机的逐盘台账）各有一张按 VM 汇总的表
「云盘 / 完成 / 失败 / 进行中 / 合计容量」，下面再逐块列出源卷、目标卷、状态与
失败原因。增量的 `ready`（基像就绪、等切换）算「进行中」——它还没换入目标卷。

两个页签**按通道分流**，不互相串数据：中转机通道不写 `vm.volumes`，所以整机走
中转机时「卷拷贝」页签会隐藏两张 RBD 表并提示"逐盘明细见「中转机通道」页签"
（混合作业会在提示里给出走中转机的 VM 台数），避免"汇总有数、卷明细却是空的"；
反过来 **VM 列表的「进度」列与 VM 详情抽屉**对中转机作业改读 `relay_disks`
（与「中转机通道」页签同一份数据，含在途实时台账），不再恒为 `—`，抽屉里多一节
「中转机通道（逐盘）」；快照/派生/排队这些还没有字节流动的阶段显示阶段文案 + 已等待时长。
...
- `MIGRATION_DERIVED_RETENTION_HOURS`：中转机通道**失败后保留派生卷/快照**
  ...
  派生；到期由对账回收，取消 / 删除任务会立即回收，页面上也可点「释放
  中间卷」马上归还配额。保留期间派生卷/快照会占用源端存储容量。
  逐盘重试的**等待集合是该 VM 上所有仍失败的盘**：页面可以只勾其中一块
  （`{"volume_ids": ["vol-..."]}`），那就只重做被勾选的盘，没勾到的失败盘继续
  留在 `awaiting_disk_retry`、页面上仍可再点，不会因为一次部分重试成功就把整台
  VM 判死；请求里的 `volume_id` 匹配不到任何失败盘时忽略这次请求并继续等待
  （不会拿别的盘顶替），等待上限仍是「失败盘等待重试 (秒)」；
