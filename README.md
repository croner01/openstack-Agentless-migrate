...
成正比（1TiB 盘几小时起步），固定阈值会把只是慢的大卷判成失败。

默认策略是"按卷大小自适应"：

- 快照等待基线 `MIGRATION_SNAPSHOT_READY_TIMEOUT`，默认 600 秒；
- 派生卷等待基线 `MIGRATION_VOLUME_READY_TIMEOUT`，默认 1800 秒；
- 实际等待上限 = `max(基线, 卷大小GiB × MIGRATION_DERIVE_SECONDS_PER_GIB)`，
  最后一项默认 20（约合 50MB/s 的全量拷贝估算）。40GiB 卷 800s、1TiB 卷
  约 5.7 小时；小于基线的卷仍按基线等。

页面「中转机配置」里的 **卷/快照就绪超时 (秒)** 可按作业覆盖：填 `0` 表示
继续按卷大小自适应，填了具体秒数则快照与派生卷都按这个值等（上限 7 天）。
同一区域的另两个参数也在这里：**停滞判死 (秒)**（数据面零字节流动多久算卡死，
对应 `relay_stall_timeout`）与 **等待空闲槽位 (秒)**（池里没有空机器的等待
上限，对应 `relay_slot_wait_seconds`）。
